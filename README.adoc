...
Using the `auto-refresh` goal it is possible to do the conversion of documents when one of them is modified.
This is useful in combination with a refresh browser extension, allowing to preview document changes while editing.
...
This accepts the same configurations as `process-asciidoc` mojo with one difference.
...
Once started, this will keep the maven process running until the commands `exit` or `quit` are inserted into the console,
or it is manually killed.

[NOTE]
====
As any Maven mojo, it can be invoked specifying it alongside the plugin prefix (`mvn asciidoctor:auto-refresh`).
These however will use the `<configuration>` applied directly to the plugin, not the ones in `<executions>`.
====
