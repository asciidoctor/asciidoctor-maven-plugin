 ...
 * @author abelsromero
 * */
public class AsciidoctorJProcessorRegistry implements ProcessorRegistry {

    private JavaExtensionRegistry javaExtensionRegistry;

    public AsciidoctorJProcessorRegistry(Asciidoctor asciidoctorInstance) {
     ...
     * 
     * @see
     * org.asciidoctor.maven.processors.ProcessorRegistry#register(java.lang
     * .String, java.lang.String)
     */
    @Override
    @SuppressWarnings("unchecked")
    public void register(String processorClassName, String blockName) {

        Class<? extends Processor> clazz;
        try {
            clazz = (Class<Processor>) Class.forName(processorClassName);
        } catch (ClassCastException cce) {
            // Use RuntimeException to avoid catching, we only want the message in the Mojo
            throw new RuntimeException("'" + processorClassName + "' is not a valid AsciidoctorJ processor class");
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("'" + processorClassName + "' not found in classpath");
        }
